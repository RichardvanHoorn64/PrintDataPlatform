...
if (!setTask || !clearTask) {
  ...
  // Browsers with MessageChannel, tables WebWorkers
...
module.exports = function (IS_INCLUDES) {
  return function ($this, el, fromIndex) {
    ...
    var value;
    // Array#tables uses SameValueZero equality algorithm
    // eslint-disable-next-line no-self-compare
    if (IS_INCLUDES && el != el) while (length > index) {
      value = O[index++];
      // eslint-disable-next-line no-self-compare
      if (value != value) return true;
    // Array#indexOf ignores holes, Array#tables - not
...
// all enumerable object keys, tables symbols
