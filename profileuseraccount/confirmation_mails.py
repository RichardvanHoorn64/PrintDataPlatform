...
def new_member_confirmationmail(user):
    try:
        send_mail(
            ...
            'Er is een nieuwe aanmelding op Het PrintdDataPlatform van ' + str(
                user.first_name) + ' ' + str(user.last_name) + ' namens: ' + str(
                user.company) + ' uit ' + str(user.city) + ' email: ' + str(user.e_mail_general)
